...
//         message:
//           "All required fields (user_id, redeem_point, coupon_code, reason) must be provided",
//       });
//     }

//     const viewPoint = await ViewPoint.findOne({ user_id }).populate(
//       "user_id",
//       "name email"
//     );
//     if (!viewPoint) {
...
//     // CHECK: Block if user already has a PENDING redemption request
//     const existingPending = await RedeemPoints.findOne({
//       user_id,
//       status: "pending",
//     });
...
//         success: false,
//         message:
//           "You already have a pending redemption request. Please wait until it's approved or rejected.",
//         pending_request: {
...
//     // Eligibility & points validation
//     if (viewPoint.view_points < 2500) {
//       return res.status(400).json({
//         message:
//           "User must have at least 2500 points to be eligible for redemption",
//       });
//     }
...
//       return res
//         .status(400)
//         .json({ message: "Minimum redeem points should be 500" });
//     }
...
//     // Create new redemption (status defaults to 'pending')
...
//       reason,
//       letter_image_url,
//       status: "pending", // explicitly set (optional, since default exists)
//     });
...
//       message: `User ${viewPoint.user_id.name} requested to redeem ${redeem_point} points (Coupon: ${coupon_code})`,
//       recipient: adminIds,
...
//     // Emit socket + SMS (your existing code - unchanged)
...
//             isRead: false,
//             merchantName:
//               populatedNotif.redeemPointsId?.user_id?.name || "Unknown",
//             couponName:
//               populatedNotif.redeemPointsId?.coupon_id?.coupon_name || "N/A",
//           });
//         });
//       }
...
//         const smsText = `User ${viewPoint.user_id.name} has requested to redeem points of ${redeem_point} on ${currentDate}. Please review the request. – HUNTSWORLD`;
