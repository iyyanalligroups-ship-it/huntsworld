...
//     if (!user_id) {
//       return res.status(400).json({ success: false, message: "user_id (seller ID) is required" });
//     }

//     const sellerId = user_id; // This is the SELLER's user_id

//     // 1. Build product query for this SELLER
//     let productQuery = { user_id: sellerId };
//     if (search) {
//       productQuery.name = { $regex: new RegExp(search, "i") };
//     }
//     if (filter) {
//       productQuery.status = filter;
//     }
...
//     // 3. 🔥 Get SELLER's PURCHASED EXTRA CITIES (main requirement)
//     const ebookPayments = await EbookPayment.find({
//       user_id: sellerId, // Seller bought these extra cities
//       payment_status: { $in: ['authorized', 'captured'] }, // Successfully paid
//       status: 'Active', // Not expired/cancelled
//       // Optional: not expired
//       $or: [
//         { access_expires_at: { $gte: new Date() } },
//         { access_expires_at: null }
//       ]
//     }).select('extra_cities payment_status total_amount createdAt').lean();

//     // 4. Flatten ALL purchased cities (unique, normalized)
//     const purchasedCitiesSet = new Set();
//     ebookPayments.forEach(payment => {
//       (payment.extra_cities || []).forEach(city => {
//         purchasedCitiesSet.add(city.trim().toLowerCase());
//       });
//     });
//     const purchasedCities = Array.from(purchasedCitiesSet).sort(); // Sorted for frontend

//     // 5. Get seller's FREE plan cities (from UserActiveFeature)
//     const activeDigitalBook = await UserActiveFeature.findOne({
//       user_id: sellerId,
...
//         { expires_at: null }
//       ]
//     }).select('selected_plan_cities').lean();

//     const freePlanCities = (activeDigitalBook?.selected_plan_cities || [])
//       .map(city => city.trim().toLowerCase())
//       .sort();

//     // 6. Response with ALL city data for frontend
...
//       // 🔥 MAIN: Purchased cities for frontend display
//       purchasedCities, // ["cuddalore", "coimbatore", "chennai"]
//       purchasedPaymentsCount: ebookPayments.length,
//       totalPaidCities: purchasedCities.length,

//       // Bonus: free plan cities
//       freePlanCities,
//       totalFreeCities: freePlanCities.length,
...
//     const total = await EbookPayment.countDocuments({ status: 'Active' });
//     
//     if (page < 1 || limit < 1) {
//       return res.status(400).json({
//         success: false,
...
//       success: true,
//       hasDigitalBook: true,
//       maxCities: feature.initial_plan_city_count || 0,
//       remainingCities: feature.remaining_plan_city_count || 0,
...
//     if (toAdd.length > feature.remaining_plan_city_count) {
//       return res.status(403).json({
//         success: false,
...
//         $inc: { remaining_plan_city_count: -toAdd.length }
//       }
//     );
...
//       remainingCities: feature.remaining_plan_city_count - toAdd.length
//     });
//   } catch (error) {
