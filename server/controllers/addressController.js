...
//     const { entity_type, address_type, address_line_1, address_line_2, city, state, country, pincode,user_id } = req.body;
//     const addressId = req.params; // Extract userId from params
//     
...
//       { _id: addressId, user_id }, // Make sure the address belongs to the user
//       {
//         entity_type,
//         address_type,
//         address_line_1,
...
//     // 1. Get Logged-in User's Primary City
//     let userCity = null;
//     const userMerchant = await Merchant.findOne({ user_id: userId }).populate('address_id').lean();

//     if (userMerchant?.address_id?.city) {
//       userCity = userMerchant.address_id.city.toLowerCase();
//     } else {
//       // Fallback to ServiceProvider or User Address if Merchant not found
//       const userAddr = await Address.findOne({ user_id: userId, address_type: 'personal' }).lean();
//       if (userAddr?.city) userCity = userAddr.city.toLowerCase();
//     }

//     // 2. Get Cities from Paid Ebook Locations
//     const ebookPayments = await EbookPayment.find({
//       user_id: userId,
//       payment_status: "paid",
//       status: "Active",
...
//       ...new Set(
//         ebookPayments.flatMap((payment) =>
//           (payment.locations || []).map((c) => c.trim().toLowerCase())
//         )
//       ),
//     ];

//     // 3. Combine both → allowedCities
//     const allowedCities = [...new Set([...(userCity ? [userCity] : []), ...purchasedCities])];

//     if (allowedCities.length === 0) {
//       return res.status(200).json({ competitors: [], hasMore: false });
//     }
...
//       { $unwind: "$userDetails" },
//       {
//         // 5. Get Latest 3 Products per Merchant
//         $lookup: {
//           from: "products",
//           let: { m_id: "$merchantDetails._id" },
//           pipeline: [
...
//     const competitors = await Address.aggregate(pipeline);
