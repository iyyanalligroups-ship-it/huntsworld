...
//     // ──────────────────────────────────────────────────────────────
//     // PHONE NUMBER VALIDATION (only when trying to change phone)
//     // ──────────────────────────────────────────────────────────────
//     if (updates.shop_phone_number !== undefined) {
//       const newPhone = updates.shop_phone_number.trim();

//       // If same phone → no need to check
//       if (newPhone === seller.shop_phone_number) {
//         // still allow update (maybe formatting change, etc.)
//       } else {
...
//         // Check if this number is already used anywhere
...
//         if (existingGrocery && existingGrocery._id.toString() !== sellerId) {
//           return res.status(400).json({
//             statusCode: 400,
//             success: false,
...
//         // If we reached here → phone is available
...
//       // We already handled shop_phone_number above
//       if (key !== "shop_phone_number") {
...
//     // -------------------------------------------------
//     // 1. Fetch GrocerySellers with pagination
//     // -------------------------------------------------
//     const sellers = await GrocerySeller.find()
//       .select(
//         "user_id shop_name shop_phone_number company_logo company_images member_type"
//       )
//       .skip(skip)
//       .limit(limit)
//       .lean();
...
//     // -------------------------------------------------
//     // 3. Fetch FULL User details + Company address
//     // -------------------------------------------------
//     const [users, addresses] = await Promise.all([
//       User.find({ _id: { $in: userIds } })
//         .select("name email profile_pic phone role createdAt isVerified")
//         .lean(),
...
//         address_type: "company",
//       })
//         .select("user_id street city state pincode")
//         .lean(),
//     ]);
...
//     // -------------------------------------------------
//     // 5. Combine everything — ONLY if user exists
//     // -------------------------------------------------
//     const result = sellers
//       .map((seller) => {
//         const user = userMap[seller.user_id];
//         const addr = addressMap[seller.user_id] || {};

//         // CRITICAL: Skip if user_id does NOT exist in User model
//         if (!user) return null;
...
//           member_type: seller.member_type,
...
//     // 6. Pagination meta
//     // -------------------------------------------------
//     const total = await GrocerySeller.countDocuments();
...
//     });
//   } catch (error) {
//     console.error("Error fetching grocery sellers:", error);
