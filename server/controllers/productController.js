...
//   try {
//     // 1. Normalise product name from URL param
//     const rawName = req.params.productId?.trim();
//     if (!rawName) {
//       return res.status(400).json({ message: "Product name is required" });
//     }
//     const productName = rawName.toLowerCase().replace(/\s+/g, "-");

//     // 2. Find verified product
//     const product = await Product.findOne({
...
//     // 4. Load user (seller → user_id)
//     const user = await User.findById(seller.user_id);
//     if (!user) {
//       return res.status(404).json({ message: "User not found for the seller" });
//     }

//     // 5. Load address (optional – can be null)
...
//     // 7. Related products (same deep sub-category, exclude self)
...
//     if (!product_name || !price || !unitOfMeasurement || !stock_quantity) {
//       return res.status(400).json({
//         success: false,
//         message: "Product name, price ,stock_quantity and unit are required",
//       });
//     }
...
//     const imageCount = Array.isArray(parsedImages) ? parsedImages.length : 0;
...
//       return res.status(400).json({
//         success: false,
//         message: "Merchant not linked to user",
//       });
//     }
...
//     // if (
//     //   !productFeature.product_limit.is_unlimited &&
//     //   productFeature.product_limit.remaining <= 0
//     // ) {
//     //   return res.status(403).json({
//     //     success: false,
//     //     message: "Product limit exceeded. Please upgrade your plan.",
//     //   });
//     // }

//     /* =========================
//        🖼️ PRODUCT PHOTO LIMIT CHECK
//     ========================= */
...
//     if (
//       imageCount > 0 &&
//       !productPhotoFeature.product_photo_limit.is_unlimited &&
//       productPhotoFeature.product_photo_limit.remaining < imageCount
//     ) {
//       return res.status(403).json({
//         success: false,
//         message: `You can upload only ${productPhotoFeature.product_photo_limit.remaining} more product image(s).`,
//       });
//     }
...
//     ========================= */
//     if (!productFeature.product_limit.is_unlimited) {
//       await UserActiveFeature.updateOne(
//         { _id: productFeature._id },
//         {
//           $inc: {
//             "product_limit.used": 1,
//             "product_limit.remaining": -1,
//           },
//         }
//       );
//     }
...
//     ) {
//       await UserActiveFeature.updateOne(
//         { _id: productPhotoFeature._id },
//         {
//           $inc: {
//             "product_photo_limit.used": imageCount,
//             "product_photo_limit.remaining": -imageCount,
//           },
//         }
//       );
//     }
...
//       const attributeDocs = parsedAttributes
...
//         const saved = await ProductAttribute.insertMany(attributeDocs);
//         product.attributes = saved.map((a) => a._id);
//         await product.save();
//       }
//     }
...
//        MARK FIRST PRODUCT
//     ========================= */
//     if (sellerModel === "Merchant" && !merchant.has_uploaded_first_product) {
//       merchant.has_uploaded_first_product = true;
...
//       product_verified_by_admin: true, // Only return verified products
//     }).populate(
//       "category_id sub_category_id seller_id super_sub_category_id deep_sub_category_id"
//     );
...
//       const oldImages = Array.isArray(product.product_image)
//         ? product.product_image
//         : [];

//       updatedFields.product_image = updatedImages;

//       // 🔥 detect only newly added images
//       const newlyAddedImages = updatedImages.filter(
//         (img) => !oldImages.includes(img)
//       );

//       const newImagesCount = newlyAddedImages.length;
...
//         if (!productPhotoFeature.product_photo_limit.is_unlimited) {

//           const totalLimit =
//             productPhotoFeature.product_photo_limit.total;

//           /* =========================
//              🔥 CALCULATE ACTUAL USED IMAGES
//           ========================= */
...
//           const totalUsed =
//             result.length > 0 ? result[0].totalUsed : 0;

//           const remaining = Math.max(totalLimit - totalUsed, 0);

//           if (remaining < newImagesCount) {
//             return res.status(403).json({
//               success: false,
//               message: `You can upload only ${remaining} more image(s).`,
//             });
//           }
//         }
//       }
...
//       updatedFields.search_tags = Array.isArray(search_tags)
//         ? search_tags
...
//     if (price !== undefined) {
//       updatedFields.price =
//         mongoose.Types.Decimal128.fromString(price.toString());
//     }
...
//         query.seller_id = { $in: merchants.map(m => m._id) };
//       }
//     }
...
//       const merchants = await Merchant.find({
//         user_id: { $in: addresses.map(a => a.user_id) },
//       }).select("_id");

//       query.seller_id = {
//         ...(query.seller_id || {}),
//         $in: merchants.map(m => m._id),
//       };
//     }
...
//     query.seller_id = {
//       ...(query.seller_id || {}),
//       $in: activeMerchants.map(m => m._id),
//     };
...
//     /* ================= 5️⃣ Add Trending + Attributes + Address ================= */
...
//     /* ================= 6️⃣ Sort ================= */
//     results.sort(
//       (a, b) => (b.totalTrendingPoints || 0) - (a.totalTrendingPoints || 0)
//     );
...
//     const activeMerchantIds = activeMerchants.map(m => m._id);
...
//     /* =====================================================
//        1️⃣ PRODUCT SUGGESTIONS
//     ===================================================== */
//     if (category === "products" && fuzzyRegex) {
//       const products = await Product.find({
//         product_verified_by_admin: true,
//         seller_id: { $in: activeMerchantIds }, // ✅ ACTIVE USERS
//         $or: [
//           { product_name: fuzzyRegex },
//           { search_tags: { $in: [fuzzyRegex] } },
//         ],
...
//       // Categories + SubCategories (no user relation)
//       const [categories, subCategories] = await Promise.all([
//         Category.find({ category_name: fuzzyRegex })
//           .select("category_name")
//           .limit(3),
//         SubCategory.find({ sub_category_name: fuzzyRegex })
...
//     const merchantType = merchantTypeMap[category];

//     if (merchantType && fuzzyRegex) {
//       const companyQuery = {
//         company_type: merchantType,
//         user_id: { $in: activeUserIds }, // ✅ ACTIVE USERS
//         $or: [
//           { company_name: fuzzyRegex },
//           { business_category: fuzzyRegex },
//         ],
...
//         companyQuery.address_id = { $in: addresses.map(a => a._id) };
//       }

//       const companies = await Merchant.find(companyQuery)
...
//           description: c.company_type,
//         })
//       );
//     }

//     /* =====================================================
//        3️⃣ BASE MEMBER (GrocerySeller) SUGGESTIONS
//     ===================================================== */
...
//         baseQuery.address_id = { $in: addresses.map(a => a._id) };
//       }

//       const baseMembers = await GrocerySeller.find(baseQuery)
...
//           description: b.member_type || "Base Member",
//         })
//       );
//     }
