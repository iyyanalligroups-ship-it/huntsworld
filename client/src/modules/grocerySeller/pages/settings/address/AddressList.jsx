...
const handleSubmit = async () => {
  const conflictingAddress = addresses?.find(
    (addr) =>
      ...
      addr._id !== selectedAddressId // This safely excludes the current one even if type didn't change
  );

  if (conflictingAddress) {
    toast.error(
      `You already have a ${formData.address_type} address. Only one ${formData.address_type} address is allowed. Please edit the existing one instead.`
    );
    return;
  }
  ...
  return (
    <div className="flex flex-col md:flex-row h-auto md:h-screen relative">
      {isProcessing && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-white/70 backdrop-blur-sm">
          <Loader label="Processing..." />
        </div>
      )}
      <div className="w-full md:w-1/2 md:min-h-screen h-100 p-4 space-y-6 border-b md:border-r">
