...
const ActivePlanCard = ({ onCancel, onUpgrade, plan: propPlan }) => {
  ...
  if (isError || error || !data) {
    return (
      <div className="flex flex-col items-center justify-center p-10 text-center text-red-600 bg-[#f0f4f6] rounded-xl border border-red-200">
        ...
        <p className="text-sm text-gray-600 mt-2">Please try again later or contact support if the issue persists.</p>
      </div>
    );
  }
  ...
  if (!data?.subscription) {
    return (
      <div className="flex flex-col items-center justify-center p-10 text-center text-red-600 bg-[#f0f4f6] rounded-xl border border-red-200">
        ...
        <p className="text-sm text-gray-600 mt-2">Please purchase a plan to access premium features.</p>
      </div>
    );
  }
  ...
  const isFreePlan = plan.subscription_plan_id.plan_code === PLAN_CODES.FREE;
  const isTrulyActive = [STATUS.PAID, STATUS.ACTIVE_RENEWAL, STATUS.ACTIVE].includes(plan.status);
  const statusWarning = !isTrulyActive ? `Warning: Plan status is "${plan.status}"` : null;
  ...
  const daysRemaining = renewalDate ? Math.max(0, Math.ceil((renewalDate - new Date()) / (1000 * 60 * 60 * 24))) : Infinity;
    ...
    <div className="relative overflow-hidden border border-indigo-100 bg-white/70 backdrop-blur-xl rounded-[2.5rem] shadow-2xl shadow-indigo-500/5 p-6 lg:p-8">
      ...
      <div className="relative z-10 space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none text-[10px] font-black uppercase tracking-widest px-2 py-0.5">Truly Active</Badge>
                {statusWarning && (
                  <Badge className="bg-amber-100 text-amber-700 hover:bg-amber-100 border-none text-[10px] font-black uppercase tracking-widest px-2 py-0.5 animate-pulse">Status Alert</Badge>
                )}
              </div>
                ...
                {!isFreePlan && (
                  <div className="flex items-baseline gap-1 mb-8">
                    ...
                    <span className="text-5xl font-black tracking-tighter">
                      {((razorpayOrder?.amount || plan.amount + plan.gst_amount) / 100).toLocaleString("en-IN")}
                    </span>
                    <span className="text-xs font-bold text-slate-400 ml-2">Total Paid</span>
                  </div>
                )}
                ...
                <div className="flex items-center gap-4 pt-6 border-t border-white/10 text-slate-400">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs font-bold text-white flex items-center gap-1.5 mt-1">
                      ...
                      {renewalDate ? `${daysRemaining} Days Left` : "Permanent Access"}
                    </span>
                  </div>
              ...
              <div className="bg-amber-50/80 backdrop-blur-sm border-2 border-amber-200 p-5 rounded-[1.5rem] flex items-start gap-4 shadow-lg shadow-amber-500/5">
                ...
                <div>
                  ...
                  <p className="text-amber-800 text-xs mt-1 leading-relaxed font-medium">
                    Your luxury access ends in <strong>{daysRemaining}</strong> days. Renew now to avoid service interruption.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="lg:col-span-7 space-y-8">
            <div>
              <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-indigo-500" /> Active Privileges
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {Object.entries(features).map(([key, value]) => (
                  <div key={key} className="bg-slate-50/50 hover:bg-white transition-colors border border-slate-100 px-4 py-3 rounded-2xl flex justify-between items-center shadow-sm">
                    ...
                    <Badge className={cn("text-[9px] font-black px-2 py-0.5 rounded-lg border-none", value === "No" || value === "Disabled" ? "bg-rose-50 text-rose-600" : "bg-emerald-50 text-emerald-600")}>{value}</Badge>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-slate-100">
          <Button
            className="group flex-1 h-14 bg-slate-900 hover:bg-indigo-600 text-white font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-slate-900/10 hover:shadow-indigo-600/20 transition-all duration-300"
            onClick={handleUpgrade}
          >
            <RefreshCw className="mr-2 h-4 w-4 group-hover:rotate-180 transition-transform duration-500" /> Manage or Upgrade Plan
          </Button>
          <Button
            variant="outline"
            className="flex-1 h-14 border-2 border-rose-100 text-rose-600 hover:bg-rose-50 font-black uppercase tracking-widest rounded-2xl transition-all duration-300"
            onClick={onCancel}
          >
            <XCircle className="mr-2 h-4 w-4" /> Terminate Subscription
          </Button>
        </div>
      </div>
      ...
      <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <DialogContent className="sm:max-w-md bg-white p-0 gap-0 overflow-hidden rounded-2xl">
          ...
          <div className="p-6 bg-slate-50 flex gap-3">
            <Button variant="outline" onClick={() => setIsConfirmOpen(false)} className="flex-1 cursor-pointer bg-white border-slate-200 hover:bg-slate-100 text-slate-700" disabled={isToggling}>Keep Auto-Pay</Button>
            <Button onClick={() => executeToggle(false)} className="flex-1 cursor-pointer bg-red-600 hover:bg-red-700 text-white" disabled={isToggling}>{isToggling ? "Disabling..." : "Yes, Disable"}</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isReenableOpen} onOpenChange={setIsReenableOpen}>
        <DialogContent className="sm:max-w-md bg-white p-0 gap-0 overflow-hidden rounded-2xl">
          ...
          <div className="p-6 bg-slate-50 flex gap-3">
            <Button variant="outline" onClick={() => setIsReenableOpen(false)} className="flex-1 cursor-pointer bg-white border-slate-200 hover:bg-slate-100 text-slate-700">Cancel</Button>
            <Button onClick={() => { setIsReenableOpen(false); onUpgrade(); }} className="flex-1 cursor-pointer bg-[#0c1f4d] hover:bg-[#0c1f4d]/90 text-white">Renew Plan</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
