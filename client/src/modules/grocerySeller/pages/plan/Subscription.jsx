...
import ActivePlanCard from './ActiveCard';
import {
  useGetAllPlansQuery,
...
const SubscriptionPlan = () => {
  ...
  useEffect(() => {
    ...
    const isActiveStatus = serverPlan && (serverPlan.status === STATUS.PAID || serverPlan.status === STATUS.ACTIVE_RENEWAL || serverPlan.status === STATUS.ACTIVE);
    ...
    const checkMemberAccess = async () => {
      ...
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/grocery-sellers/fetch-member-type`,
          {
  ...
  useEffect(() => {
    const checkAddressOnLoad = async () => {
      ...
      try {
        ...
        const fallbackMessage = "Please complete your address details before purchasing a plan";
        setAddressErrorMessage(serverMessage || fallbackMessage);
  ...
  const confirmCancel = async () => {
    ...
    try {
      ...
      // Extra safety: force clear again shortly after refetch
      setTimeout(() => {
        setLocalActivePlan(null);
      }, 600);

      // Reset local flag after some time
      setTimeout(() => setJustCancelledLocally(false), 8000);
    } catch (error) {
  ...
  if (isLoading || isActiveSubscriptionLoading) {
    return (
      <div className={`flex-1 min-h-[60vh] flex items-center justify-center transition-all duration-300 ${isSidebarOpen ? "ml-0 sm:ml-64" : "ml-0 sm:ml-16"}`}>
        <Loader contained={true} label="Securely Loading Subscription..." />
      </div>
    );
  }
      ...
      <div className="relative overflow-hidden bg-[#0a0a0f] pb-32 pt-20 lg:pt-24">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="text-xl leading-relaxed text-slate-400 max-w-2xl mx-auto font-medium">
            A high-performance infrastructure for grocery sellers who demand precision and scalability. No limits, just exponential growth.
          </p>
        </div>
      </div>
                        ...
                        return (
                          <li key={elem.feature_id} className="flex items-start gap-3 group/item">
                            ...
                            <div className="flex flex-col">
                              <span className="text-[14px] leading-snug tracking-tight text-slate-700 font-bold">
                                {elem.feature_name}
                              </span>
                              {!isDisabled && elem.value?.data && elem.value.data !== "Enable" && (
                                <span className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.15em] mt-1 opacity-90 drop-shadow-sm">
