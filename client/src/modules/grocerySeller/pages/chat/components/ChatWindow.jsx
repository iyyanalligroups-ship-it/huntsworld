...
export default function ChatWindow() {
  ...
  useEffect(() => {
    if (isBottomSuccess && bottomData?.data) {
      ...
      setMessages((prev) => {
        ...
        console.log("useGetMessagesQuery new messages:", newMessages);
    ...
    const handleReceiveMessage = (msg) => {
      ...
      if (isRelated && msg._id) {
        setMessages((prev) => {
          ...
          // Check for optimistic message match by content, sender, receiver, and timestamp
          ...
          if (optimisticMatch) {
            ...
            console.log("Updating optimistic message:", optimisticMatch._id, "to", msg._id);
            return prev
              .map((m) =>
                m._id === optimisticMatch._id
                  ? { ...m, _id: msg._id, createdAt: msg.createdAt, isOptimistic: false }
                  : m
              )
              .sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
          }
          // Add new message if no optimistic match and not already present
          if (!existingIds.has(msg._id)) {
            console.log("Adding new message to state:", msg);
            return [...prev, { ...msg, fromMe: msg.sender === user?.user?._id }].sort(
              (a, b) => new Date(a.createdAt) - new Date(b.createdAt)
            );
          }
          console.log("Message already exists:", msg._id);
          return prev;
        });
  ...
  const handleEdit = (msg) => {
    ...
    setEditMessage({ content: msg.content });
    setIsEditing(true);
  };
  ...
  return (
    <div className="flex flex-col overflow-y-scroll flex-1 h-full">
      {opponentTyping && (
        <div className="text-sm text-gray-500 italic mb-2 px-4">
              ...
              >
                <div
                  className={`px-4 py-2 rounded-xl max-w-xs shadow relative mb-2 ${
                    msg.fromMe ? "bg-[#d8fdd1] text-black" : "bg-white text-black"
                  } group`}
                >
                  {msg.deleted ? (
