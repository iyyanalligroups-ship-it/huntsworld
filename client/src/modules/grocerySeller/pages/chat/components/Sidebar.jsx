...
//   const { data, isFetching, isSuccess } = useGetAllUsersQuery(
//     {
...
//                   lastMessageDeleted: false,
//                   unreadCount:
//                     u?._id === msg.receiver && u?._id !== selectedUserId
//                       ? (u?.unreadCount || 0) + 1
//                       : u?.unreadCount || 0,
//                 }
//               : u
...
//       if (userId === user?.user?._id && selectedUserId === selectedUserId) {
//         setAllUsers((prev) =>
//           prev.map((u) =>
...
//   );
// }
// src/modules/chat/components/Sidebar.jsx
import { useContext, useEffect, useRef, useState } from "react";
import { Input } from "@/components/ui/input";
import {
  useGetAllUsersQuery,
...
export default function Sidebar() {
  ...
  console.log("GrocerySeller Sidebar: rendered");
  ...
  // Socket listeners - EXACT same as User Panel
  useEffect(() => {
    if (!socket || !user?.user?._id) return;
    ...
    const handleReceiveMessage = (msg) => {
      ...
      if (isRelated) {
        const contactId =
          msg.sender === user?.user?._id ? msg.receiver : msg.sender;
        setAllUsers((prev) => {
        ...
        setAllUsers((prev) => {
          const updatedUsers = prev.map((u) => {
            if (u?._id === contactId) {
              const isLatest =
                !u.lastMessageTime ||
                new Date(updatedMsg.updatedAt).getTime() >=
          ...
          const updatedUsers = prev.map((u) => {
            if (u?._id === contactId) {
              const isLatest =
                ...
                Math.abs(
                  new Date(u.lastMessageTime).getTime() -
                  new Date(deletedMsg.createdAt).getTime()
                ) <= 1000;
              console.log("isLatest check:", {
                isLatest,
                lastMessageTime: u.lastMessageTime,
                deletedMsgCreatedAt: deletedMsg.createdAt,
              });
              if (isLatest) {
                return {
                  ...u,
                  lastMessage: "This message was deleted",
                  lastMessageTime:
                    deletedMsg.updatedAt || new Date().toISOString(),
                  lastMessageDeleted: true,
                };
              }
            }
  ...
  return (
    <div className="w-72 bg-sidebar border-2 h-full flex flex-col p-2">
      <Input
      ...
      >
        {filteredUsers.length > 0 ? (
          ...
          <p className="text-center text-xs text-sidebar-foreground py-2">
            {searchTerm ? "No customers found" : "No users available"}
          </p>
        )}
        {isFetching && (
          <p className="text-center text-xs text-sidebar-foreground py-2">
