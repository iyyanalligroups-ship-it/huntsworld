...
export default function Header() {
  ...
  const entityType = fetchedUserData?.entityType || (selectedUser?.user_id ? 'Grocery Seller' : 'Buyer');

  const { data: lastMsgData, isLoading: isMsgLoading, isError: isMsgError, error: msgError } = useGetLastMessageBetweenUsersQuery(
    {
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <Badge variant="outline">{userInfo?.referral_code || 'N/A'}</Badge>
                      </div>
                      <Separator />
                      <div className="flex justify-between">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>{userInfo?.role?.role || userInfo?.role || 'N/A'}</span>
                      </div>
                      <Separator />
                      <div className="flex justify-between">
                        <span className="font-medium">Number Verified:</span>
                        <Badge variant={userInfo?.number_verified ? "default" : "destructive"}>
                        ...
                        {addressInfo && Object.keys(addressInfo || {}).length > 0 ? (
                          <div className="text-xs mt-1 text-slate-700">
                            {addressInfo?.address_line_1 && (
                              <>{addressInfo.address_line_1}{addressInfo?.address_line_2 ? "," : ""} </>
                            )}
