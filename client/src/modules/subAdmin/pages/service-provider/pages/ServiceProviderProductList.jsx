...
const ServiceProviderList = ({ products,pagination, onEdit }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (deleteProductResponse.success == true) {
        showToast(deleteProductResponse.message || "Product Deleted Successfully",'success');
      }else{
        showToast(deleteProductResponse.message || "Failed to Delete",'error');
      }
  
    } catch (err) {
      showToast(err.data.message || `Error during deletion: ${err}`,'error')
