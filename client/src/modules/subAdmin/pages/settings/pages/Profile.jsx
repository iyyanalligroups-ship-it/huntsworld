...
const Profile = ({ userId }) => {
  ...
  const handleCopyReferral = () => {
    const referralLink = `http://localhost:5173/referral-register?ref=${userDetails?.referral_code}`;
  ...
  const handleSave = async () => {
    try {
       const response = await updateUser({ id: userId, updatedUser: formData }).unwrap();
      setIsEditing(false);
      if (response.success) {
