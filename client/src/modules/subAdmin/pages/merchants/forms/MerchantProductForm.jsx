...
const StepperProductForm = ({ editingProduct, onClose }) => {
  ...
  useEffect(() => {
    if (editingProduct) {
      setFormData({
        ...
        sellerModel: "Merchant",
        product_name: editingProduct.product_name || "",
        price: editingProduct.price?.$numberDecimal || editingProduct.price || "",
        stock_quantity: editingProduct.stock_quantity || "",
        description: editingProduct.description || "",
        product_image: editingProduct.product_image || [],
      });
  ...
  const handleImageDelete = async (urlsToDelete = []) => {
    ...
    try {
      const deleteRes = await deleteImage({
        file_names: fileNames,
        product_name: formData.product_name || editingProduct?.product_name,
      }).unwrap();
      const updatedImages = (formData.product_image || []).filter(
        (url) => !urlsToDelete.includes(url)
      );
  ...
  const handleSubmit = async (e) => {
    ...
    const productPayload = {
      ...formData,
      ...categoryData,
      attributes: attributes.filter((attr) => attr.key && attr.value), // Filter out empty attributes
      seller_id: selectedMerchant._id,
      sellerModel: "Merchant",
    };
  ...
  return (
    <>
      <h1 className="font-bold text-xl border-b-2">{editingProduct ? "Edit Product" : "Add Product"}:</h1>
      <form className="p-6 bg-white rounded-2xl shadow-md max-w-2xl mx-auto">
        <div className="flex justify-between mb-6">
          {["Category", "Basic Info", "Attributes"].map((label, index) => (
            <div
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <Select
              ...
              disabled={!categoryData.category_id}
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Select Sub Category" />
              </SelectTrigger>
            ...
            <Select
              ...
              disabled={!categoryData.sub_category_id}
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Select Super Sub Category" />
              </SelectTrigger>
            ...
            <Select
              ...
              disabled={!categoryData.super_sub_category_id}
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Select Deep Sub Category" />
              </SelectTrigger>
