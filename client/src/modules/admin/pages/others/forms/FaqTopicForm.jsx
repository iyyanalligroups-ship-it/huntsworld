    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (selectedTopic) {
                ...
                toast.success(response.message || "Faq Topic Updated Successfully");
                    
                } else {
                    toast.error(response.message || "Failed to Update Faq Topic");
                }
            } else {
                const response = await createFaqTopic(data).unwrap();
                if (response) {
                    console.log(response);
                toast.success(response.message || "Faq Topic Created Successfully");
                    
                } else {
                    toast.error(response.message || "Failed to Create Faq Topic");
                }
            }
            onSuccess();
            setTitle("");
