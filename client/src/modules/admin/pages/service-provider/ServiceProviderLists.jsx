...
const ProviderDetails = ({ provider }) => (
  <div>
    <h3 className="text-lg font-semibold mb-2 text-white">Provider Details</h3>
    <p className="text-white"><strong>User ID:</strong> {provider.user_id?._id || "-"}</p>
    <p className="text-white"><strong>Email:</strong> {provider.company_email || "-"}</p>
    <p className="text-white"><strong>Phone Number:</strong> {provider.company_phone_number || "-"}</p>
    <p className="text-white"><strong>Verified:</strong> {provider.verified_status ? "Yes" : "No"}</p>
    <p className="text-white"><strong>Trust Shield:</strong> {provider.trust_shield ? "Yes" : "No"}</p>
    <p className="text-white"><strong>Travels Name:</strong> {provider.travels_name || "-"}</p>
    <p className="text-white"><strong>License Number:</strong> {provider.license_number || "-"}</p>
    <p className="text-white"><strong>Number of Vehicles:</strong> {provider.number_of_vehicles || "-"}</p>
    <p className="text-white"><strong>Vehicle Type:</strong> {provider.vehicle_type || "-"}</p>
    <p className="text-white"><strong>Description:</strong> {provider.description || "-"}</p>
    <p className="text-white"><strong>Company Logo:</strong> {provider.company_logo ? <img src={provider.company_logo} alt="Logo" className="w-20 h-20" /> : "-"}</p>
    <p className="text-white"><strong>Company Images:</strong> {provider.company_images?.length > 0 ? provider.company_images.join(", ") : "-"}</p>
    <p className="text-white"><strong>Created At:</strong> {new Date(provider.created_at).toLocaleString() || "-"}</p>
    <p className="text-white"><strong>Updated At:</strong> {new Date(provider.updated_at).toLocaleString() || "-"}</p>
    <p className="text-white"><strong>Address:</strong> {provider.address_id ? `${provider.address_id.address_line1}, ${provider.address_id.city}, ${provider.address_id.state}, ${provider.address_id.country}, ${provider.address_id.postal_code}` : "-"}</p>
  </div>
);

const AdditionalDetails = ({ provider }) => (
  <div>
    <h3 className="text-lg font-semibold mb-2 text-white">Additional Details</h3>
    <p className="text-white">Additional information for {provider.company_email || "this provider"} can be added here.</p>
  </div>
);
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent>
                        ...
                        <DropdownMenuItem onClick={() => handleEdit(provider._id)}>
                          Edit
                        </DropdownMenuItem>
