...
const SubDealerDetails = ({ subDealer }) => {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto p-4 sm:p-6 md:p-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl md:text-2xl">Company Overview</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h3 className="text-base sm:text-lg md:text-xl font-semibold">{subDealer?.company_name || 'N/A'}</h3>
              <p className="text-sm sm:text-base text-muted-foreground">{subDealer?.description || 'No description available'}</p>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl md:text-2xl">User Information</CardTitle>
        </CardHeader>
        <CardContent>
          {userLoading ? (
            <div className="flex items-center justify-center">
              <Loader2 className="h-5 w-5 sm:h-6 sm:w-6 animate-spin" />
              <p className="ml-2 text-sm sm:text-base text-muted-foreground">Loading user details...</p>
            </div>
          ) : userError ? (
            <p className="text-sm sm:text-base text-red-500">Failed to load user details</p>
          ) : (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl md:text-2xl">Address</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl md:text-2xl">Merchant Details</CardTitle>
        </CardHeader>
        <CardContent>
          {merchantLoading ? (
            <div className="flex items-center justify-center">
              <Loader2 className="h-5 w-5 sm:h-6 sm:w-6 animate-spin" />
              <p className="ml-2 text-sm sm:text-base text-muted-foreground">Loading merchant details...</p>
            </div>
          ) : merchantError ? (
            <p className="text-sm sm:text-base text-red-500">Failed to load merchant details</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl md:text-2xl">Products</CardTitle>
        </CardHeader>
        <CardContent>
          {products.length > 0 ? (
              ...
              <Table>
                ...
                <TableBody>
                  {products.map((product, index) => (
                    <TableRow key={index}>
                      <TableCell className="text-sm">{product.name || 'N/A'}</TableCell>
                      <TableCell className="text-sm">{product.description || 'N/A'}</TableCell>
                      <TableCell className="text-sm">{product.price ? `$${product.price}` : 'N/A'}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            ...
            <p className="text-sm sm:text-base text-muted-foreground">No products available</p>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl md:text-2xl">Company Images</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {subDealer.company_images.map((url, index) => (
                <img
                  ...
                  className="h-24 sm:h-32 md:h-40 w-full object-cover rounded-md"
                />
              ))}
            </div>
          ) : (
