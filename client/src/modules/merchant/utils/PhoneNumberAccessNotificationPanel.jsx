      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100">
        <div className="flex items-center">
          ...
          <h3 className="font-medium text-sm">
            Phone Number Access Requests {notifications.filter(n => !n.is_read).length > 0 && `(${notifications.filter(n => !n.is_read).length})`}
          </h3>
        </div>
            ...
            {notifications.map((notification) => (
              <div
                ...
                className={`p-3 border-b border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors duration-200 relative ${
                  notification.is_read ? 'bg-white' : 'bg-blue-50/70 border-l-4 border-blue-500'
                }`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center mt-1 text-xs text-gray-500">
                      <span>{formatTime(notification.createdAt || notification.created_at || notification.request_date)}</span>
                    </div>
                  </div>
                  {!notification.is_read && (
                    <span className="h-2 w-2 rounded-full bg-blue-600 absolute top-4 right-4"></span>
                  )}
                </div>
              </div>
            ))}
