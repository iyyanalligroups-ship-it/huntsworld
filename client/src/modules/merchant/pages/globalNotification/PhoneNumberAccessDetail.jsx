...
const PhoneNumberAccessDetail = () => {
  ...
  console.log("PhoneNumberAccessDetail: id =", id);
  ...
  useEffect(() => {
    if (isError) {
      console.error("PhoneNumberAccessDetail: API Error:", error);
    }
    if (data) {
      console.log("PhoneNumberAccessDetail: Data loaded:", data);
    }
  }, [data, isError, error]);
  ...
  if (isError || !data) {
    ...
    return (
      <div className={`${isSidebarOpen ? 'ml-1 sm:ml-56' : 'ml-1 sm:ml-16'} p-4`}>
        <div>Error loading phone number access request details</div>
      </div>
    );
  }
  ...
  return (
    <div className={`${isSidebarOpen ? 'ml-1 sm:ml-56' : 'ml-1 sm:ml-16'} p-4 lg:p-8 bg-slate-50 min-h-screen font-sans`}>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <Button
          variant="ghost"
          className="hover:bg-slate-200/50 cursor-pointer text-slate-600 transition-colors mb-2"
              ...
              {isPending && (
                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                  <Button 
                    ...
                    className="flex-1 h-12 cursor-pointer bg-green-600 hover:bg-green-700 text-white font-bold text-md shadow-lg shadow-green-600/20 transition-all active:scale-95 flex items-center justify-center gap-2"
                  >
                    <Check size={20} />
                    Approve Access
                  </Button>
                  <Button 
                    ...
                    variant="outline"
                    className="flex-1 cursor-pointer h-12 border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300 font-bold text-md transition-all active:scale-95 flex items-center justify-center gap-2"
                  >
                    <X size={20} />
                    Decline Request
                  </Button>
