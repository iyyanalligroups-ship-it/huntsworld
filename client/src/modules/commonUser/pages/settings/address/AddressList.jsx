...
import { Alert, AlertDescription } from "@/components/ui/alert"; // Assuming you have Alert component
...
const AddressList = () => {
  ...
  // === NEW: Check for duplicate address_type ===
  const existingAddressOfType = addresses.find(
    (addr) =>
      addr.address_type === formData.address_type &&
      addr._id !== selectedAddressId // Allow same type when editing the same address
  );

  const isAddDisabled = !selectedAddressId && formData.address_type && existingAddressOfType;
  ...
  const handleSubmit = async () => {
    if (!selectedAddressId && existingAddressOfType) {
      toast.error(`You already have a ${formData.address_type} address. Only one is allowed.`);
      return;
    }
  ...
  if (isLoading) return <Loader label="Loading addresses..." />;

  return (
    <>
      {isAdding && <Loader label="Adding Address..." />}
      {isUpdating && <Loader label="Updating Address..." />}
      {isDeleting && <Loader label="Deleting Address..." />}
      ...
      <div className="w-full md:w-1/2 p-4">
        ...
        {/* Warning if trying to add duplicate type */}
        {isAddDisabled && (
          <Alert variant="destructive" className="mb-4">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              You already have a <strong>{formData.address_type}</strong> address.
              Only one address per type is allowed. Edit the existing one instead.
            </AlertDescription>
          </Alert>
        )}
